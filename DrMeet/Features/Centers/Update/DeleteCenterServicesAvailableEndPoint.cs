...
public static class DeleteCenterServicesAvailableEndPoint
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapPost($"{ApiInfo.Prefix}/DeleteServicesAvailable", handler: async (
                    ICenterService service,
                    [FromBody] ToggleCenterServicesAvailableDto request
                ) =>
            {
                var result = await service.DeleteCenterServicesAvailableAsync(request);
