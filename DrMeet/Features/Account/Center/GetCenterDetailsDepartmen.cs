...
public static class GetCenterDetailsDepartmen
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet($"{ApiInfo.Prefix}/GetCenterDetailsDepartmen", handler: async (
                ...
                ) =>
            {
                ...
                {
                    return Ok(new { userType = result.userType, user = await centerService.GetCenterDepartmen(result.id) });
                }
                else
