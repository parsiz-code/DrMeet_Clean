...
public static class GetCenterDetailsContractedSupplementalInsurances
{
    public class EndPoint : BaseEndpoint, IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet($"{ApiInfo.Prefix}/GetCenterDetailsContractedSupplementalInsurances", handler: async (
                ...
                ) =>
            {
                ...
                {
                    return Ok(new { userType = result.userType, user = await centerService.GetCenterContractedSupplementalInsurances(result.id, request) });
                }
                else
